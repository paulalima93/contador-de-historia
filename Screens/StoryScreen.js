...
export default class StoryScreen extends Component {
  ...
  async initiateTTS(title, author, story, moral){
    const corrent_color = this.state.speakerColor;
    this.setState({
        speakerColor: corrent_color === "gray" ? "#ee8249" : "gray"
    });
    if(corrent_color === "gray"){
      Speech.speak(`${title} criado por ${author}`)
      Speech.speak(story);
    ...
    if (!this.props.route.params) {
      ...
      return (
        <View style={this.state.light_theme ? styles.containerLight: styles.container}>
          ...
          <View style={this.state.light_theme ? styles.appTitleTextLight: styles.appTitle}>
